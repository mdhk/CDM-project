"""
Adjusted NLTK corpus reader for the XML version of the CHILDES corpus
    1) added speaker IDs to returned utterances
    2) made converted age more precise (added partial months)
lines changed from original are marked with "### EDITED".

Original: https://www.nltk.org/_modules/nltk/corpus/reader/childes.html
"""
...
class CHILDESCorpusReader(XMLCorpusReader):
    ...
    def convert_age(self, age_year):
        ...
        try:
            ### EDITED add partial months as fraction
            # if int(m.group(3)) > 15:
            #     age_month += 1
            if int(m.group(3)) > 0:
                age_month += float(m.group(3)) / 30.42
            ###
    ...
    def _getMLU(self, fileid, speaker):
        ...
        numFillers = 0
        sentDiscount = 0
        ### EDITED line below (added spkID)
        for spkID, sent in sents:
        ###
    ...
    def _get_words(
        ...
        for xmlsent in xmldoc.findall(".//{%s}u" % NS):
            ### EDITED line below to add speaker ID
                ...
                for xmlword in xmlsent.findall(".//{%s}w" % NS):
                    ...
                    if relation == True:
                        ...
                        try:
                            ...
                            pass
                    ### EDITED line below to add word to second element in list (sentence)
                    sents[1].append(word)
                    ###
                if sent or relation:
                    results.append(sents)
                else:
                    ### EDITED line below to ignore speaker ID when not returning sentences
                    results.extend(sents[1])
                    ###
